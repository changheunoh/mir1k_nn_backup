function [cepstra,aspectrum,pspectrum] = mymelfcc(samples, sr, varargin)
...
% [pspectrum,logE] = powspec(samples, sr, wintime, hoptime, dither);
% [spect_mix, ~, ~] = choh_stft(samples, param.window_size, param.hop_size, param.fft_size, param.fs);
[spect_mix, ~, ~, dB_spect] = choh_stft(samples, sr*wintime, sr*hoptime, sr*wintime, sr);
winpts = round(wintime*sr);
pspectrum = abs(spect_mix).^2;
    if (dither)
        pspectrum = pspectrum + winpts;
    end
